...
export default function DocumentQuery() {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const completedDocs = response.data.filter(
        (doc: Document) => doc.status === 'completed' && doc.vector_db_indexed
      )
  ...
  const handleQuery = async () => {
    ...
    if (!selectedDocId) {
      setValidationError('Please select a processed document first.')
      return
    }
    if (!trimmed) {
    ...
    if (trimmed.length < 3) {
      setValidationError('Query is too short. Add more detail.')
      return
    }
    if (trimmed.length > 500) {
      setValidationError('Query too long. Keep under 500 characters.')
      return
    }
    setValidationError('')
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-12rem)] flex flex-col">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">Query Documents</h1>
        <p className="text-gray-400">Ask questions about your processed documents using AI</p>
      </div>
      ...
      <div>
        ...
        {loadingDocs ? (
          ...
          <div className="p-4 bg-yellow-900/20 border border-yellow-500/50 rounded-lg text-yellow-400">
          ...
          <select
            ...
            className="w-full px-4 py-3 bg-gray-800 border border-gray-700 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
          >
            {documents.map(doc => (
              <option key={doc.id} value={doc.id}>
                {doc.filename} {doc.chunks_indexed && doc.chunks_indexed > 0 && `(${doc.chunks_indexed} chunks)`}
              </option>
            ))}
          </select>
        )}
        ...
        <div className="border-t border-gray-800 p-4">
          <div className="flex items-end space-x-2">
            <div className="flex-1 relative">
              <textarea
                ...
                className="w-full px-4 py-3 pr-12 bg-gray-800 border border-gray-700 text-white rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent resize-none disabled:bg-gray-900 disabled:cursor-not-allowed placeholder-gray-500"
          ...
          {selectedDoc && (
            <p className="text-xs text-gray-400 mt-2">
              Querying: <span className="font-medium text-gray-300">{selectedDoc.filename}</span>
            </p>
          )}
        </div>
      </div>
